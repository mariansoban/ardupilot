...
void AP_LTM::send_LTM(uint8_t LTPacket[], uint8_t LTPacket_size){
    //calculate Checksum
    uint8_t LTCrc = 0x00;
...
// Sensors frame
void AP_LTM::generate_Sframe(void){
    ...
    send_LTM(LTBuff, LTM_SFRAME_SIZE);
    _ltm_scheduler++;
}

// Attitude frame
void AP_LTM::generate_Aframe(void){
	...
	send_LTM(LTBuff, LTM_AFRAME_SIZE);
    _ltm_scheduler++;
}

// Send LTM
void AP_LTM::generate_LTM(void){
        if (_ltm_scheduler & 1) {    // is odd
            generate_Aframe();
        } else {					// is even
            if (_ltm_scheduler % 4 == 0) generate_Sframe();
            else generate_Gframe();
        }
